...
# 「该」用负向后顾排除「应该」（副词，非指代）——否则含「应该」的界外问题被误判含指代、
# 压制 oos 组合谓词（审查修订 D-C-A1）。「该函数/该模块」等真指代仍命中。
_RE_PRONOUN = re.compile(r"(它|这个|这块|那块|那个|(?<!应)该|上面|上文|前面|刚才|之前那|上述)")
...
# §3.2 S7 保守偏差）**落 flag**；**可定位但断言的支撑关系缺失**（missing_edge 半支）→ 本轮
# 词面校验保守视为已获证据、**不产 flag**，留待边级校验（避免误伤真前提，与 verify_premises
# docstring 及 spec §5.7 一致）。flag 触发生成层「先纠正后答」的程序化闸门（spec §5.2）。
#
...
_NUM_UNIT_RE = re.compile(r"[0-9一二三四五六七八九十百千零两]+\s*(?:级|轮|层|阶段|个|次|步|档|重)")
