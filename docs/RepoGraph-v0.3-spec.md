...
| **F9（新增）** | **无向量层的召回上界**：长尾中文口语在 BM25+改写后仍可能 miss（FZ 天然难集，Concept 别名近乎全空） | 新增 · **Phase D·V5 已量化（FZ-test 词面不可达 2/10=0.20）** | §4.2 双语卡片入 BM25 语料 + `zh_aliases` 是主要补偿；FZ hit@k 如实报告 v0.1→v0.2 增量，不承诺向量级召回。**【Phase D·V5 实测 @HEAD c971078 / tag phase-d-20260723】** FZ-test（冻结留出 10 题）hit@3=**0.8**；2 失败题（t03/t04）经真实 BM25 全量排名逐题归因**全部为「词面不可达」**（gold 富化卡片与查询词元零 n-gram 交集——暂停↔stop、记号↔标记仅单字重叠不成 2-gram 词元）、**0 排序失利、0 其他**。⇒ 向量层缺失净代价 = **2/10 = 0.20**（词面不可达占 FZ-test 比例；失败内构成 100%）。FZ-dev 交叉核验 3 失败 = 2 词面不可达（d06/d09）+ 1 排序失利（d10：gold 实现 `apply_gate_decision` 命中「放行」但 rank-4 被挤出 top3），与 D-11 C2 归因逐题一致。数字为 **lexical 下界**（S2 改写 / L2 llm 增益未计入）。产物 `design_work/d3_f9.{py,json}`；承 D-P4。 |
